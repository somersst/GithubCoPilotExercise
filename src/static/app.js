      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (details.participants.length === 0) {
          ...
          details.participants.forEach(email => {
            participantsHTML += `<li class="participant-item"><span class="participant-email">${email}</span><span class="delete-participant" title="Unregister" data-activity="${name}" data-email="${email}">&#128465;</span></li>`;
          });
        }
